...
use App\Models\Seles_matrial_types;
use App\Http\Controllers\Controller;
use App\Http\Requests\SalesMatiralTypesRequest;

class Seles_matrial_typesController extends Controller
{
    public function index()
            {
            $data = Seles_matrial_types::select()->orderby('id', 'DESC')->paginate(PAGINATION_COUNT);
            if (!empty($data)) {
            ...
            //check if not exsits
            $checkExists = Seles_matrial_types::where(['name' => $request->name, 'com_code' => $com_code])->first();
            if ($checkExists == null) {
            ...
            Seles_matrial_types::create($data);
            ...
            {
            $data = Seles_matrial_types::select()->find($id);
            ...
            $checkExists = Seles_matrial_types::where(['name' => $request->name, 'com_code' => $com_code])->where('id', '!=', $id)->first();
            if ($checkExists != null) {
            return redirect()->back()
            ...
            Seles_matrial_types::where(['id' => $id, 'com_code' => $com_code])->update($data_to_update);
            ...
            {
            try {
            $Sales_matrial_types_row = Seles_matrial_types::find($id);
